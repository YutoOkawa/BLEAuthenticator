...
/* a=1/a mod 2^BIGBITS_B1024_28. This is very fast! */
void B1024_28::BIG_invmod2m(BIG a)
{
    int i;
    BIG U, t1, b, c;
    BIG_zero(U);
    BIG_inc(U, invmod256(BIG_lastbits(a, 8)));
    for (i = 8; i < BIGBITS_B1024_28; i <<= 1)
    {
        BIG_norm(U);
        BIG_copy(b, a);
    ...
    BIG_copy(a, U);
    BIG_norm(a);
    BIG_mod2m(a, BIGBITS_B1024_28);
}

/* x=y */
void RSA2048::FF_copy(BIG x[], BIG y[], int n)
{
    int i;
    for (i = 0; i < n; i++)
        BIG_copy(x[i], y[i]);
...
/* x=y<<n */
static void RSA2048::FF_dsucopy(BIG x[], BIG y[], int n)
{
    int i;
    for (i = 0; i < n; i++)
    {
...
/* x=y */
static void RSA2048::FF_dscopy(BIG x[], BIG y[], int n)
{
    int i;
    for (i = 0; i < n; i++)
    {
...
/* x=y>>n */
static void RSA2048::FF_sducopy(BIG x[], BIG y[], int n)
{
    int i;
    for (i = 0; i < n; i++)
        BIG_copy(x[i], y[n + i]);
...
/* set to zero */
void RSA2048::FF_zero(BIG x[], int n)
{
    int i;
    for (i = 0; i < n; i++)
        BIG_zero(x[i]);
...
/* test equals 0 */
int RSA2048::FF_iszilch(BIG x[], int n)
{
    int i;
    for (i = 0; i < n; i++)
...
/* shift right by BIGBITS_B1024_28-bit words */
static void RSA2048::FF_shrw(BIG a[], int n)
{
    int i;
    for (i = 0; i < n; i++)
    {
...
/* shift left by BIGBITS_B1024_28-bit words */
static void RSA2048::FF_shlw(BIG a[], int n)
{
    int i;
    for (i = 0; i < n; i++)
    {
...
/* extract last bit */
int RSA2048::FF_parity(BIG x[])
{
    return BIG_parity(x[0]);
}

/* extract last m bits */
int RSA2048::FF_lastbits(BIG x[], int m)
{
    return BIG_lastbits(x[0], m);
}

/* x=1 */
void RSA2048::FF_one(BIG x[], int n)
{
    int i;
    BIG_one(x[0]);
    for (i = 1; i < n; i++)
...
void RSA2048::FF_init(BIG x[], sign32 m, int n)
{
    int i;
    BIG_zero(x[0]);
#if CHUNK<64
    x[0][0] = (chunk)(m & BMASK_B1024_28);
    x[0][1] = (chunk)(m >> BASEBITS_B1024_28);
#else
    x[0][0] = (chunk)m;
#endif
    for (i = 1; i < n; i++)
...
int RSA2048::FF_comp(BIG x[], BIG y[], int n)
{
    int i, j;
    for (i = n - 1; i >= 0; i--)
    {
...
/* recursive add */
static void RSA2048::FF_radd(BIG z[], int zp, BIG x[], int xp, BIG y[], int yp, int n)
{
    int i;
    for (i = 0; i < n; i++)
...
/* recursive inc */
static void RSA2048::FF_rinc(BIG z[], int zp, BIG y[], int yp, int n)
{
    int i;
    for (i = 0; i < n; i++)
...
/* recursive dec */
static void RSA2048::FF_rdec(BIG z[], int zp, BIG y[], int yp, int n)
{
    int i;
    for (i = 0; i < n; i++)
...
/* simple add */
void RSA2048::FF_add(BIG z[], BIG x[], BIG y[], int n)
{
    int i;
    for (i = 0; i < n; i++)
        BIG_add(z[i], x[i], y[i]);
...
/* simple sub */
void RSA2048::FF_sub(BIG z[], BIG x[], BIG y[], int n)
{
    int i;
    for (i = 0; i < n; i++)
        BIG_sub(z[i], x[i], y[i]);
...
void RSA2048::FF_inc(BIG x[], int m, int n)
{
    BIG_inc(x[0], m);
    FF_norm(x, n);
}

void RSA2048::FF_dec(BIG x[], int m, int n)
{
    BIG_dec(x[0], m);
    FF_norm(x, n);
}
...
static void RSA2048::FF_rnorm(BIG z[], int zp, int n)
{
    int i, trunc = 0;
    chunk carry;
    if (n < 0)
    ...
    for (i = 0; i < n - 1; i++)
    {
        ...
        z[zp + i][NLEN_B1024_28 - 1] ^= carry << P_TBITS_RSA2048; /* remove it */
        z[zp + i + 1][0] += carry;
    }
    carry = BIG_norm(z[zp + n - 1]);
    if (trunc) z[zp + n - 1][NLEN_B1024_28 - 1] ^= carry << P_TBITS_RSA2048;
}

void RSA2048::FF_norm(BIG z[], int n)
{
    FF_rnorm(z, 0, n);
}

/* shift left by one bit */
void RSA2048::FF_shl(BIG x[], int n)
{
    int i;
    ...
    for (i = 0; i < n - 1; i++)
    {
        ...
        x[i][0] |= delay_carry;
        x[i][NLEN_B1024_28 - 1] ^= (chunk)carry << P_TBITS_RSA2048;
        delay_carry = carry;
    }
    BIG_fshl(x[n - 1], 1);
    x[n - 1][0] |= delay_carry;
...
void RSA2048::FF_shr(BIG x[], int n)
{
    int i;
    int carry;
    for (i = n - 1; i > 0; i--)
    {
        ...
        x[i - 1][NLEN_B1024_28 - 1] |= (chunk)carry << P_TBITS_RSA2048;
    }
    BIG_fshr(x[0], 1);
}

void RSA2048::FF_output(BIG x[], int n)
{
    int i;
    FF_norm(x, n);
    for (i = n - 1; i >= 0; i--)
...
void RSA2048::FF_rawoutput(BIG x[], int n)
{
    int i;
    for (i = n - 1; i >= 0; i--)
    {
...
void RSA2048::FF_toOctet(octet *w, BIG x[], int n)
{
    int i;
    w->len = n * MODBYTES_B1024_28;
    for (i = 0; i < n; i++)
    {
        BIG_toBytes(&(w->val[(n - i - 1)*MODBYTES_B1024_28]), x[i]);
    }
}

void RSA2048::FF_fromOctet(BIG x[], octet *w, int n)
{
    int i;
    for (i = 0; i < n; i++)
    {
        BIG_fromBytes(x[i], &(w->val[(n - i - 1)*MODBYTES_B1024_28]));
    }
}

/* in-place swapping using xor - side channel resistant */
static void RSA2048::FF_cswap(BIG a[], BIG b[], int d, int n)
{
    int i;
    for (i = 0; i < n; i++)
        BIG_cswap(a[i], b[i], d);
...
static void RSA2048::FF_karmul(BIG z[], int zp, BIG x[], int xp, BIG y[], int yp, BIG t[], int tp, int n)
{
    int nd2;
    if (n == 1)
    {
        ...
        BIG_split(z[zp + 1], z[zp], t[tp], BIGBITS_B1024_28);
        return;
    }
...
static void RSA2048::FF_karsqr(BIG z[], int zp, BIG x[], int xp, BIG t[], int tp, int n)
{
    int nd2;
    if (n == 1)
    {
...
static void RSA2048::FF_karmul_lower(BIG z[], int zp, BIG x[], int xp, BIG y[], int yp, BIG t[], int tp, int n)
{
...
static void RSA2048::FF_karmul_upper(BIG z[], BIG x[], BIG y[], BIG t[], int n)
{
...
/* z=x*y */
void RSA2048::FF_mul(BIG z[], BIG x[], BIG y[], int n)
{
#ifndef C99
    BIG t[2 * FFLEN_RSA2048];
#else
    BIG t[2 * n];
#endif
...
static void RSA2048::FF_lmul(BIG z[], BIG x[], BIG y[], int n)
{
#ifndef C99
    BIG t[2 * FFLEN_RSA2048];
#else
...
/* Set b=b mod c */
void RSA2048::FF_mod(BIG b[], BIG c[], int n)
{
    int k = 0;
...
/* z=x^2 */
void RSA2048::FF_sqr(BIG z[], BIG x[], int n)
{
#ifndef C99
    BIG t[2 * FFLEN_RSA2048];
#else
...
static void RSA2048::FF_reduce(BIG r[], BIG T[], BIG N[], BIG ND[], int n)
{
    ...
    BIG m[n];
#endif
    RSA2048::FF_sducopy(r, T, n); /* keep top half of T */
...
/* a is of length - 2*n */
/* r,b is of length - n */
void RSA2048::FF_dmod(BIG r[], BIG a[], BIG b[], int n)
{
    int k;
#ifndef C99
    BIG m[2 * FFLEN_RSA2048];
    BIG x[2 * FFLEN_RSA2048];
#else
    BIG m[2 * n];
    BIG x[2 * n];
#endif
...
void RSA2048::FF_invmodp(BIG r[], BIG a[], BIG p[], int n)
{
#ifndef C99
    BIG u[FFLEN_RSA2048], v[FFLEN_RSA2048], x1[FFLEN_RSA2048], x2[FFLEN_RSA2048], t[FFLEN_RSA2048], one[FFLEN_RSA2048];
#else
...
/* nesidue mod m */
static void RSA2048::FF_nres(BIG a[], BIG m[], int n)
{
#ifndef C99
    BIG d[2 * FFLEN_RSA2048];
#else
    BIG d[2 * n];
#endif
    if (n == 1)
    {
        BIG_dscopy(d[0], a[0]);
        BIG_dshl(d[0], NLEN_B1024_28 * BASEBITS_B1024_28);
...
static void RSA2048::FF_redc(BIG a[], BIG m[], BIG ND[], int n)
{
#ifndef C99
    BIG d[2 * FFLEN_RSA2048];
#else
    ...
    if (n == 1)
    {
        BIG_dzero(d[0]);
        BIG_dscopy(d[0], a[0]);
        BIG_monty(a[0], m[0], ((chunk)1 << BASEBITS_B1024_28) - ND[0][0], d[0]);
    }
    else
    {
        FF_mod(a, m, n);
...
static void RSA2048::FF_invmod2m(BIG U[], BIG a[], int n)
{
    int i;
#ifndef C99
    BIG t1[2*FFLEN_RSA2048], b[FFLEN_RSA2048], c[FFLEN_RSA2048];
#else
    BIG t1[2 * n], b[n], c[n];
#endif
...
void RSA2048::FF_random(BIG x[], csprng *rng, int n)
{
    int i;
    for (i = 0; i < n; i++)
    {
    ...
    while (BIG_nbits(x[n - 1]) < MODBYTES_B1024_28 * 8) BIG_random(x[n - 1], rng);
}

/* generate random x mod p */
void RSA2048::FF_randomnum(BIG x[], BIG p[], csprng *rng, int n)
{
    int i;
#ifndef C99
    BIG d[2 * FFLEN_RSA2048];
...
static void RSA2048::FF_modmul(BIG z[], BIG x[], BIG y[], BIG p[], BIG ND[], int n)
{
#ifndef C99
    BIG d[2 * FFLEN_RSA2048];
#else
    BIG d[2 * n];
#endif
    chunk ex = P_EXCESS_RSA2048(x[n - 1]);
    chunk ey = P_EXCESS_RSA2048(y[n - 1]);
#ifdef dchunk
    if ((dchunk)(ex + 1) * (ey + 1) > (dchunk)P_FEXCESS_RSA2048)
#else
    if ((ex + 1) > P_FEXCESS_RSA2048 / (ey + 1))
#endif
    {
#ifdef DEBUG_REDUCE
    ...
    {
        BIG_mul(d[0], x[0], y[0]);
        BIG_monty(z[0], p[0], ((chunk)1 << BASEBITS_B1024_28) - ND[0][0], d[0]);
    }
    else
    {
        FF_mul(d, x, y, n);
...
static void RSA2048::FF_modsqr(BIG z[], BIG x[], BIG p[], BIG ND[], int n)
{
#ifndef C99
    BIG d[2 * FFLEN_RSA2048];
#else
    ...
    chunk ex = P_EXCESS_RSA2048(x[n - 1]);
#ifdef dchunk
    if ((dchunk)(ex + 1) * (ex + 1) > (dchunk)P_FEXCESS_RSA2048)
#else
    if ((ex + 1) > P_FEXCESS_RSA2048 / (ex + 1))
#endif
    {
#ifdef DEBUG_REDUCE
...
void RSA2048::FF_skpow(BIG r[], BIG x[], BIG e[], BIG p[], int n)
{
    int i, b;
#ifndef C99
    BIG R0[FFLEN_RSA2048], R1[FFLEN_RSA2048], ND[FFLEN_RSA2048];
#else
    BIG R0[n], R1[n], ND[n];
#endif
    FF_invmod2m(ND, p, n);
    ...
    for (i = 8 * MODBYTES_B1024_28 * n - 1; i >= 0; i--)
    {
        b = BIG_bit(e[i / BIGBITS_B1024_28], i % BIGBITS_B1024_28);
...
void RSA2048::FF_skspow(BIG r[], BIG x[], BIG e, BIG p[], int n)
{
    int i, b;
#ifndef C99
    BIG R0[FFLEN_RSA2048], R1[FFLEN_RSA2048], ND[FFLEN_RSA2048];
    ...
    FF_nres(R0, p, n);
    FF_nres(R1, p, n);
    for (i = 8 * MODBYTES_B1024_28 - 1; i >= 0; i--)
    {
        b = BIG_bit(e, i);
        FF_modmul(r, R0, R1, p, ND, n);
        FF_cswap(R0, R1, b, n);
...
void RSA2048::FF_power(BIG r[], BIG x[], int e, BIG p[], int n)
{
    int f = 1;
#ifndef C99
    BIG w[FFLEN_RSA2048], ND[FFLEN_RSA2048];
#else
    BIG w[n], ND[n];
#endif
    FF_invmod2m(ND, p, n);
...
void RSA2048::FF_pow(BIG r[], BIG x[], BIG e[], BIG p[], int n)
{
    int i, b;
#ifndef C99
    BIG w[FFLEN_RSA2048], ND[FFLEN_RSA2048];
...
void RSA2048::FF_pow2(BIG r[], BIG x[], BIG e, BIG y[], BIG f, BIG p[], int n)
{
    int i, eb, fb;
#ifndef C99
    BIG xn[FFLEN_RSA2048], yn[FFLEN_RSA2048], xy[FFLEN_RSA2048], ND[FFLEN_RSA2048];
#else
...
int RSA2048::FF_cfactor(BIG w[], sign32 s, int n)
{
    int r;
    sign32 g;
#ifndef C99
    BIG x[FFLEN_RSA2048], y[FFLEN_RSA2048];
#else
    BIG x[n], y[n];
#endif
    FF_init(y, s, n);
...
#if CHUNK<32
    g = x[0][0] + ((sign32)(x[0][1]) << BASEBITS_B1024_28);
#else
    g = (sign32)x[0][0];
#endif
    r = igcd(s, g);
...
int RSA2048::FF_prime(BIG p[], csprng *rng, int n)
{
    int i, j, loop, s = 0;
#ifndef C99
    BIG d[FFLEN_RSA2048], x[FFLEN_RSA2048], unity[FFLEN_RSA2048], nm1[FFLEN_RSA2048];
#else
